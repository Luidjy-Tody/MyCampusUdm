...
if ($_SERVER["REQUEST_METHOD"] === "POST")
{
  ...
  $checkClub = $pdo->prepare("SELECT id FROM clubs WHERE id = :club_id AND responsable_id = :responsable_id AND statut <> 'supprime'");
  $checkClub->execute([
    "club_id" => $clubPostId,
    "responsable_id" => $responsableId
  ]);
  ...
  if ($action === "create")
  {
    ...
    $membresNotif = $pdo->prepare("SELECT utilisateur_id FROM membres_club WHERE club_id = :club_id");
    $membresNotif->execute(["club_id" => $clubPostId]);
    foreach ($membresNotif->fetchAll(PDO::FETCH_COLUMN) as $membreId)
    {
      createNotification($pdo, (int) $membreId, "Nouvel événement", "Un nouvel événement a été publié dans l’un de vos clubs : " . $titre, "evenement");
    }
