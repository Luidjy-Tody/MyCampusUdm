...
$statsStatement = $pdo->prepare("SELECT
    ...
    (SELECT COUNT(*) FROM evenements e
      ...
      WHERE mc.utilisateur_id = :user_id AND e.statut = 'actif' AND e.date_event >= CURDATE()) AS evenements_a_venir
");
$statsStatement->execute(["user_id" => $userId]);
$stats = $statsStatement->fetch(PDO::FETCH_ASSOC);

$clubsStatement = $pdo->prepare("
  SELECT c.id, c.nom_club, c.description, mc.role_membre, mc.date_entree
      ...
      <tbody>
        <?php foreach ($mesClubs as $club) : ?>
          <tr>
            ...
            <td><a class="bouton petit" href="club_detail.php?id=<?= (int) $club["id"] ?>">Voir</a></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
